...
/* ------------------------------------------------------------------
   Analogital Pocket
   写真を選ぶ → レシピ／明るさ／トリミングを調整 → 別ファイルで保存
...
// ---- 状態 ----
let originalCanvas = null;              // 向き補正済み・フル解像度の元画像（読み取り専用）
let cropRect = { x: 0, y: 0, w: 0, h: 0 }; // 元画像座標での切り抜き範囲
let exposure = 0;                       // 明るさ（-100〜+100）
let currentRecipe = null;               // null = 元の写真

// ---- レシピ（recipes-data.js のデータから組み立て）----
...
// ---- 明るさ＋レシピをまとめて当てる（1回の読み書きで高速）----
function applyAdjustments(ctx, w, h) {
  if (exposure === 0 && !currentRecipe) return;
  const img = ctx.getImageData(0, 0, w, h);
  if (exposure !== 0) {
    const f = Math.pow(2, exposure / 100); // -100→0.5倍, +100→2倍
    const d = img.data;
...
// ---- プレビュー（切り抜き範囲を画面サイズに縮小して表示）----
function setupPreview() {
  const maxW = Math.min(window.innerWidth - 32, 1000);
  const maxH = Math.min(window.innerHeight * 0.52, 1400);
  const sw = cropRect.w, sh = cropRect.h;
...
function renderPreview() {
  ...
  pctx.drawImage(originalCanvas, cropRect.x, cropRect.y, cropRect.w, cropRect.h, 0, 0, W, H);
  applyAdjustments(pctx, W, H);
}
...
/* ==================================================================
   トリミング（切り抜き）
   ・crop中は previewCanvas に元画像全体＋切り抜き枠を描画
   ・枠内ドラッグで移動、四隅ハンドルでサイズ変更、比率ロック対応
================================================================== */
let cropMode = false;
let cropScale = 1;            // 表示座標 × cropScale = 元画像座標
let cropRatio = 'free';       // 'free' または 幅/高 の数値
let box = { x: 0, y: 0, w: 0, h: 0 }; // 表示座標での枠
let drag = null;
...
function enterCropMode() {
  ...
  statusEl.textContent = '枠を動かして範囲を決め、「適用」を押してください。';
  drawCrop();
}
...
function drawCrop() {
  const W = previewCanvas.width, H = previewCanvas.height;
  pctx.clearRect(0, 0, W, H);
  pctx.drawImage(originalCanvas, 0, 0, W, H);
  // 枠外を暗くする
  pctx.save();
  pctx.fillStyle = 'rgba(0,0,0,.5)';
  pctx.beginPath();
  pctx.rect(0, 0, W, H);
  pctx.rect(box.x, box.y, box.w, box.h);
  pctx.fill('evenodd');
  pctx.restore();
  // 枠線＋三分割ガイド
  pctx.strokeStyle = 'rgba(255,255,255,.95)';
  pctx.lineWidth = 2;
...
function resizeBox(type, px, py) {
  ...
  else { fx = box.x + box.w; fy = box.y; } // bl
  ...
  if (cropRatio !== 'free') {
    ...
    w = Math.min(w, availW); h = Math.min(h, availH);
  }
...
function setRatio(rStr) {
  ...
  if (cropRatio !== 'free' && originalCanvas) {
    const W = previewCanvas.width, H = previewCanvas.height;
...
previewCanvas.addEventListener('pointerdown', (e) => {
  ...
  drag = { type: t, lastX: p.x, lastY: p.y };
  e.preventDefault();
});
...
saveBtn.addEventListener('click', async () => {
  ...
  try {
    ...
    octx.drawImage(originalCanvas, cropRect.x, cropRect.y, cropRect.w, cropRect.h, 0, 0, cropRect.w, cropRect.h);
